  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="quotationModalLabel">{{ __('messages.create') }} {{ __('messages.quotation') }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <div class="modal-body" style="padding-top: 10px; padding-bottom: 5px;">
        <form  id="quotation_create_form" name="quotation_create_form" method="POST" action="{{ route('quotations.store') }}">
          @csrf
          <div id="quotation_create_modal"></div>
        </form>
      </div>
...
@push('scripts')
  <script>
    $('#quotation_create_form').on('submit', function(e) {
      e.preventDefault(); 
      $.ajax({
    ...
    // quotation details
    function AddRow() {
      var tbody = document.querySelector('.quotation-table tbody');
