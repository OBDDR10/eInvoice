                ...
                <table id="dt-table" class="table align-items-center justify-content-center mb-0">
                  <thead>
                    <tr class="bg-color-light-gray">
                      ...
                      <th class="text-sm sortable" data-column="invoice_no">{{ __('messages.invoice_no') }}</th>
...
@section('script')
  <script>
    $(document).ready(function() {
        reloadInvoiceTable(entries,page,getQueryParams());

        // change entry
        $('#entries').on('change', function() {
            var entries = $(this).val();
            reloadInvoiceTable(entries,1,getQueryParams(sorting));
        });
    ...
    //     reloadInvoiceTable(10,1,getQueryParams());
    // });
    ...
    $(".sortable").on('click', function() {
        ...
        reloadInvoiceTable(entries,page,getQueryParams(sortObj));
    });

    // reload table
    function reloadInvoiceTable(entries, page = 1, queryParams = null) {
      $.ajax({
          url: '{{ route("invoices.index") }}?' + queryParams,
          type: 'GET',
    ...
    $('#CreateModal').on('show.bs.modal', function (event) {
        ...
        $.ajax({
            ...
            data: { id: 0 },
            success: function(data) {
                $('#invoice_create_modal').html(data.modal);
            },
            error: function(xhr, status, error) {
                console.error(xhr.responseText);
            }
    ...
    $('#DetailModal').on('show.bs.modal', function (event) {
        ...
        $.ajax({
            ...
            data: { id: data_id },
            success: function(data) {
                $('#invoice_detail_modal').html(data.modal);
            },
            error: function(xhr, status, error) {
                console.error(xhr.responseText);
            }
    ...
    //         data: { id: data_id },
    //         success: function(data) {
    //             $('#invoice_edit_modal').html(data.modal);
    //         },
    //         error: function(xhr, status, error) {
    //             console.error(error);
    //         }
