  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="sendModalLabel">{{ __('messages.send') }} {{ __('messages.invoice') }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <div class="modal-body text-center">
        <form id="send_form" name="send_form" method="POST" action="{{ route('invoices.send') }}">
          @csrf
          <input type="hidden" id="send_id" name="send_id" value="" />
          <i class="material-icons text-info" style="font-size: 100px;">send</i>
          <p class="text-black text-md font-weight-md pt-3">Are you sure you want to send this invoice?</p>
          <p class="text-sm font-weight-md">You are sending this invoice to</p>
