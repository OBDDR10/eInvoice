  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">{{ __('messages.delete') }} {{ __('messages.transaction') }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <div class="modal-body text-center">
        <form id="transaction_delete_form" name="transaction_delete_form" method="POST" action="{{ route('transactions.destroy') }}">
          @csrf
...
@push('scripts')
  <script>
    ...
    $('#submit_delete_form').on('click', function() {
        $('#transaction_delete_form').submit();
    });
  </script>
@endpush